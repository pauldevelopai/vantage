...
def test_full_bridge_flow(client):
    ...
    r = client.post("/api/cameras/bridge/pair")
    assert r.status_code == 200
    code = r.json()["code"]
    assert code

    # 2. Agent registers with the code (no admin auth — public register).
    r = client.post("/api/cameras/bridge/register",
    ...
    r = client.get("/api/cameras/bridge")
    ...
    r = client.post(f"/api/cameras/bridge/{bridge_id}/scan", json={"cidr": "192.168.1.0/24"})
    assert r.status_code == 200
    job_id = r.json()["job_id"]

    # 5. Agent polls for the job (with its token) and claims it.
    r = client.get("/api/cameras/bridge/jobs", headers=agent_headers)
    job = r.json()["job"]
    ...
    r = client.post(f"/api/cameras/bridge/jobs/{job_id}/results",
                    headers=agent_headers, json={"cameras": cams})
    assert r.status_code == 200

    # 7. Admin reads the finished job + results.
    r = client.get(f"/api/cameras/bridge/scan/{job_id}")
    body = r.json()
...
def test_agent_endpoints_reject_bad_token(client):
    r = client.get("/api/cameras/bridge/jobs",
                   headers={"X-Bridge-Id": "brg_nope", "X-Bridge-Token": "bad"})
    assert r.status_code == 401


def test_agent_endpoints_require_credentials(client):
    r = client.get("/api/cameras/bridge/jobs")   # no headers
    assert r.status_code == 401


def test_register_rejects_bad_code(client):
    r = client.post("/api/cameras/bridge/register", json={"code": "NOPE", "name": "x"})
...
def test_scan_offline_bridge_is_conflict(client):
    ...
    code = client.post("/api/cameras/bridge/pair").json()["code"]
    bridge_id = client.post("/api/cameras/bridge/register",
                            json={"code": code, "name": "x"}).json()["bridge_id"]
    bridge_mod._registry.get_bridge(bridge_id).last_seen = None

    r = client.post(f"/api/cameras/bridge/{bridge_id}/scan", json={})
